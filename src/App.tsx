...
function AppContent() {
  ...
  if (isAdminMode) {
    return (
      <div className="min-h-screen bg-[#141517] text-gray-200 font-sans selection:bg-emerald-500/30 pb-20 lg:pb-0">
        ...
        <BottomMenu currentView={currentView} onNavigate={setCurrentView} />
      </div>
    );
  }
  ...
  const hasVipAccess = useMemo(() => {
    ...
    if (userProfile.expires && new Date(userProfile.expires) < new Date()) return false;
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  {currentView === 'free'
                    ...
                    : currentView === 'safe'
                    ? 'High confidence VIP selections'
              ...
              sortedDates.map(date => (
                <div key={date} className="space-y-4">
                  <div className="flex items-center gap-2 px-1">
                    ...
                    <h3 className="text-emerald-500 font-bold text-sm uppercase tracking-wider">{date}</h3>
                  </div>
